...
class SelfHealingImputer:
    ...
    def impute_from_physics(self, reading: AWSReading, faulty_channel: str) -> Optional[float]:
        ...
        if faulty_channel == "humidity_pct" and tdew is not None:
            # RH = 100 * exp((a * Tdew) / (b + Tdew) - (a * T) / (b + T))
            try:
        ...
        elif faulty_channel == "temperature_c" and tdew is not None and rh > 5.0:
            # T estimation from Tdew and RH
            try:
                gamma_dew = (self.a * tdew) / (self.b + tdew)
    ...
    def correct_reading(
        ...
        spatial_consensus: Optional[Dict[str, float]] = None,
        temporal_fallback: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        """
        ...
        channels_to_fix = [affected_channel] if affected_channel in corrected else list(corrected.keys())

        for ch in channels_to_fix:
            imputed_val = None

            # Priority 1: Spatial neighbor consensus
            if spatial_consensus is not None and ch in spatial_consensus:
            ...
            if imputed_val is None and temporal_fallback is not None and ch in temporal_fallback:
