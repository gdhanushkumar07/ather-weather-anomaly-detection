    ...
    <div className="section-card historical-graphs-card">
      ...
      <div className="card-header-flex">
        <div className="card-title-group">
          ...
          <span className="card-section-title">READINGS & CHARTS</span>
          <span
            className="modelled-reference-tag"
            title="Only the latest point is the station's current reading. Earlier points are generated by the backend from a diurnal model — they are not recorded AWS telemetry history."
          >
            MODELLED / SYNTHETIC REFERENCE
          </span>
        </div>
        ...
        <div className="graph-controls-cluster">
          ...
          <button
            ...
            title={showTable ? 'Hide data table' : 'Show data table (modelled reference values)'}
          >
                ...
                {series.slice().reverse().map((p, idx) => (
                  <div key={idx} className="table-data-row">
                    ...
                    <span className="td-col status">
                      <span className={`table-status-pill ${idx === 0 ? (station.status === 'ANOMALY' ? 'anomaly' : 'normal') : 'modelled'}`}>
                        {idx === 0 ? station.status : 'MODELLED'}
                      </span>
                    </span>
                  </div>
                ))}
